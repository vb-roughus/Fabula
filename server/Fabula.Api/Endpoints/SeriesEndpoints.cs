    ...
    public static IEndpointRouteBuilder MapSeriesEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (FabulaDbContext db, CancellationToken ct) =>
            await db.Series
                .AsNoTracking()
                .OrderBy(s => s.Name)
                .Select(s => new SeriesSummaryDto(s.Id, s.Name, s.Description, s.Books.Count))
                .ToListAsync(ct));
        ...
        group.MapPost("/", async (FabulaDbContext db, SeriesRequest req, CancellationToken ct) =>
        {
            ...
            return Results.Created($"/api/series/{series.Id}",
                new SeriesSummaryDto(series.Id, series.Name, series.Description, 0));
        });
        ...
        group.MapPut("/{id:int}", async (int id, FabulaDbContext db, SeriesRequest req, CancellationToken ct) =>
        {
            ...
            return Results.Ok(new SeriesSummaryDto(series.Id, series.Name, series.Description, count));
        });
...
public record SeriesSummaryDto(int Id, string Name, string? Description, int BookCount);
